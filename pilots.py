...
# choose from and return a pilot_id and pilot_name. Passes the relevant columns and pilot_id to display all
...
# pilot cannot be added (license numbers must be unique), so a relevant message is diaplyed before returning to 
...
def add_pilot():
    ...
    if cursor.fetchone():
        ...
        conn.close()
        return
    cursor.execute("INSERT INTO pilots (first_name, last_name, license_number) VALUES (?, ?, ?)", (first_name, last_name, license_number))
    conn.commit()
...
# to receive a valid name or license number from the user. When the license number is being updated, the database is queried 
# to check a pilot with the provided license number does not already exist. If the license number already exists, the
# pilot cannot be added (license numbers must be unique), so a relevant message is displayed before returning to 
